...
export async function getAcademicStats({ tenantId, grade, term, curriculum = 'CBC' }) {
  console.log(`[Analytics] Starting getAcademicStats for tenant:${tenantId}, grade:${grade}, term:${term}, curr:${curriculum}`);
  try {
    ...
    const expectedSubjectsCount = (subjCfg && subjCfg[grade] && subjCfg[grade].length > 0) 
    ...
    console.log(`[Analytics] Completed successfully. Entered learners: ${enteredLearners}/${expectedCount}`);
