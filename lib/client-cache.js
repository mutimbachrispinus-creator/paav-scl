...
import { idbGet, idbSet, addToOutbox } from './idb';
...
function getTenantId() {
  ...
  try {
    ...
    if (raw) {
      ...
      return user?.tenantId || 'platform-master';
    }
  } catch {}
...
function store(key, value, serverTs = 0) {
  ...
  try {
    ...
    // Mirror to IndexedDB for persistence
    idbSet(key, entry);
  } catch {}
}
...
export async function mutateDB(key, value, url = '/api/db', body = null) {
  ...
  if (typeof window !== 'undefined') {
    ...
    const fullKey = `${tid}_${cacheKey}`;
    const stamp = Date.now();
    const entry = { v: value, t: stamp, s: stamp };
    localStorage.setItem(PREFIX + fullKey, JSON.stringify(entry));
    window.dispatchEvent(new CustomEvent('paav:sync', { detail: { changed: [key] } }));
  }
