...
const DB_TTL   = 60 * 1000;        // 60 seconds
...
function read(key, ttl) {
  ...
  try {
    ...
    // return stale value even after TTL — caller decides what to do with it
    if (Date.now() - t > ttl) return { stale: true, value: v };
    return { stale: false, value: v };
  } catch { return null; }
}
...
export function prefetchKeys(keys) {
  if (typeof window === 'undefined') return;
  const missing = keys.filter(k => !sessionStorage.getItem(PREFIX + 'db_' + k));
