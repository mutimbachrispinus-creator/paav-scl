...
import { PAAV_KEYS } from './constants';
...
async function flushOutbox() {
  const { getOutbox, deleteFromOutbox } = await import('./idb');
  ...
  for (const action of actions) {
    try {
      ...
      if (res.ok) {
        await deleteFromOutbox(action.id);
      }
    } catch (e) {
...
export async function sync() {
  try {
    ...
    const local = await getLocalTimestamps(PAAV_KEYS);
