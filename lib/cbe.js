...
export function calcLearnerReportData(marks, adm, grade, term, subjects, cfg = null, curriculum = 'CBC', weights = null) {
  ...
  const w = weights || { op1: 1/3, mt1: 1/3, et1: 1/3 };
  ...
  for (const s of subjects) {
    ...
    // Calculate weighted average based on available marks
    const activeWeights = {
      op1: op !== null ? (w.op1 || 0) : 0,
      mt1: mt !== null ? (w.mt1 || 0) : 0,
      et1: et !== null ? (w.et1 || 0) : 0
    };
    const weightSum = activeWeights.op1 + activeWeights.mt1 + activeWeights.et1;
    
    const avg = weightSum > 0 
      ? Number((((op||0)*activeWeights.op1 + (mt||0)*activeWeights.mt1 + (et||0)*activeWeights.et1) / weightSum).toFixed(2))
      : 0;
