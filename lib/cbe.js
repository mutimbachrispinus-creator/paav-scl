...
export function gInfo(score, grade, cfg = null) {
  const useJSS = isJSSGrade(grade);

  // Admin-configured custom scale takes priority
  if (cfg) {
    const scale = useJSS
      ? (cfg.jss    || JSS_SCALE)
      : (cfg.pri    || PRIMARY_SCALE);
  ...
  // Default scales
  if (useJSS) {
    const entry = JSS_SCALE.find(x => score >= x.min) || JSS_SCALE[JSS_SCALE.length - 1];
    return { lv: entry.lv, pts: entry.pts, c: entry.c, bg: entry.bg, desc: entry.desc };
  }

  const entry = PRIMARY_SCALE.find(x => score >= x.min) || PRIMARY_SCALE[PRIMARY_SCALE.length - 1];
  return { lv: entry.lv, pts: entry.pts, c: entry.c, bg: entry.bg, desc: entry.desc };
}
