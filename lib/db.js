...
export async function kvGetManyWithMeta(keys, tenantId = 'platform-master') {
  ...
  // Handle special case for staff which is in its own table
  const hasStaff = keys.includes('paav6_staff');
  let staffData = null;
  let staffUpdatedAt = 0;
  if (hasStaff) {
    const [staffRows, staffMeta] = await Promise.all([
      query('SELECT * FROM staff WHERE tenant_id = ?', [tenantId]),
      query('SELECT updated_at FROM kv WHERE key = ? AND tenant_id = ?', ['paav6_staff', tenantId])
    ]);
    staffData = staffRows.map(rowToStaff);
    staffUpdatedAt = staffMeta[0]?.updated_at || 0;
  }

  const placeholders = keys.filter(k => k !== 'paav6_staff').map(() => '?').join(',');
  let rows = [];
  if (placeholders) {
    rows = await query(`SELECT key, value, updated_at FROM kv WHERE key IN (${placeholders}) AND tenant_id = ?`, [...keys.filter(k => k !== 'paav6_staff'), tenantId]);
  }
  ...
  return keys.map(k => {
    if (k === 'paav6_staff') {
      return { key: k, value: staffData || [], updatedAt: staffUpdatedAt };
    }
