...
export async function kvSet(key, value) {
  ...
  if (key === 'paav6_learners') {
    ...
    for (const l of value) {
      stmts.push({
        sql: `INSERT INTO learners (adm, name, grade, sex, age, dob, stream, teacher, parent, phone, addr, t1, t2, t3) 
              VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        args: [l.adm, l.name, l.grade, l.sex, l.age, l.dob, l.stream, l.teacher, l.parent, l.phone, l.addr, l.t1, l.t2, l.t3]
      });
    }
    await batch(stmts);
  ...
  if (key === 'paav6_staff') {
    ...
    for (const s of value) {
      stmts.push({
        sql: `INSERT INTO staff (id, name, username, role, phone, password, status, childAdm, grade, teachingAreas, secQ, secA, email, createdAt)
              VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        args: [s.id, s.name, s.username, s.role, s.phone, s.password, s.status, s.childAdm, s.grade, JSON.stringify(s.teachingAreas || []), s.secQ, s.secA, s.email, s.createdAt]
      });
    }
    await batch(stmts);
    await execute(`INSERT INTO kv (key, updated_at) VALUES (?, strftime('%s','now')) ON CONFLICT(key) DO UPDATE SET updated_at = excluded.updated_at`, [key]);
