 ...
 */
export async function kvRecordPayment({ adm, term, amount, method, ref, by }) {
  await ensureSchema();
  const termCol = String(term).toLowerCase(); // t1, t2, t3
  if (!['t1', 't2', 't3'].includes(termCol)) throw new Error('Invalid term');

  // 1. Update learner table
  await execute(`UPDATE learners SET ${termCol} = ${termCol} + ? WHERE adm = ?`, [Number(amount), adm]);
