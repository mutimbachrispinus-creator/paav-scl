...
/**
 * Delete a payment and reverse its effect on the learner's balance.
 */
export async function kvDeletePayment(paymentId) {
  await ensureSchema();
  
  // 1. Get payment details
  const pRows = await query('SELECT * FROM paylog WHERE id = ?', [paymentId]);
  if (!pRows.length) return;
  const p = pRows[0];
  
  // 2. Get learner details
  const lRows = await query('SELECT * FROM learners WHERE adm = ?', [p.adm]);
  if (lRows.length) {
    const l = lRows[0];
    let remaining = Number(p.amount);
    const updates = { t1: l.t1 || 0, t2: l.t2 || 0, t3: l.t3 || 0, arrears: l.arrears || 0 };
    
    // Reverse allocation: subtract from T3, then T2, then T1, then add back to arrears
    const terms = ['t3', 't2', 't1'];
    for (const t of terms) {
      if (remaining <= 0) break;
      const val = updates[t];
      if (val > 0) {
        const sub = Math.min(remaining, val);
        updates[t] -= sub;
        remaining -= sub;
      }
    }
    if (remaining > 0) {
      updates.arrears += remaining;
    }
    
    await execute(
      `UPDATE learners SET t1 = ?, t2 = ?, t3 = ?, arrears = ? WHERE adm = ?`,
      [updates.t1, updates.t2, updates.t3, updates.arrears, l.adm]
    );
  }
  
  // 3. Delete from paylog
  await execute('DELETE FROM paylog WHERE id = ?', [paymentId]);
  
  // 4. Touch kv timestamps
  await execute(`INSERT INTO kv (key, updated_at) VALUES (?, strftime('%s','now')) ON CONFLICT(key) DO UPDATE SET updated_at = excluded.updated_at`, ['paav6_paylog']);
  await execute(`INSERT INTO kv (key, updated_at) VALUES (?, strftime('%s','now')) ON CONFLICT(key) DO UPDATE SET updated_at = excluded.updated_at`, ['paav6_learners']);
}
