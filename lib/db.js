...
function getClient() {
  ...
  if (!process.env.TURSO_URL || !process.env.TURSO_TOKEN) {
    throw new Error('Database connection failed: TURSO_URL or TURSO_TOKEN is not defined in the environment. Please check your .env file or hosting provider settings.');
  }
...
export async function ensureSchema() {
  ...
  // Alterations (wrapped in try-catch blocks)
  // Note: batch doesn't support individual try-catches easily, 
  // so we run these separately for now, but we could also check if column exists.
...
export async function kvSet(key, value) {
  ...
  if (key === 'paav6_learners') {
    ...
    for (const l of value) {
      stmts.push({
        sql: `INSERT INTO learners (adm, name, grade, sex, age, dob, stream, teacher, parent, phone, parentEmail, addr, t1, t2, t3, avatar) 
              VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
        args: [l.adm, l.name, l.grade, l.sex, l.age, l.dob, l.stream, l.teacher, l.parent, l.phone, l.parentEmail || null, l.addr, l.t1, l.t2, l.t3, l.avatar || null]
      });
    }
    await batch(stmts);
...
export async function kvRecordPayment({ adm, term, amount, method, ref, by, status = 'approved' }) {
  ...
  if (status === 'approved' && l) {
    ...
    const updates = { t1: l.t1 || 0, t2: l.t2 || 0, t3: l.t3 || 0 };
    ...
    for (const t of termsToFill) {
      ...
      if (limit > 0 && current < limit) {
        ...
        // If no limit set or already full, move to next term UNLESS it's the last term
        if (t === 't3') {
          updates[t] += remaining; // Overpayment on the very last term if everything else is full
          remaining = 0;
        }
      }
    }
    
    // If there's still remaining (e.g. they paid more than the entire annual fee)
    if (remaining > 0) updates.t3 += remaining;

    await execute(
      `UPDATE learners SET t1 = ?, t2 = ?, t3 = ? WHERE adm = ?`,
      [updates.t1, updates.t2, updates.t3, adm]
    );
  }

  // 2. Add to paylog (KV)
  const paylog = await kvGet('paav6_paylog') || [];
  // (learners and l already fetched above)
  ...
  const json = JSON.stringify(paylog);
  await execute(
    `INSERT INTO kv (key, value, updated_at) VALUES (?, ?, strftime('%s','now'))
     ON CONFLICT(key) DO UPDATE SET value = excluded.value, updated_at = excluded.updated_at`,
    ['paav6_paylog', json]
  ...
  // also touch learners kv so frontend knows to refresh
  await execute(`INSERT INTO kv (key, updated_at) VALUES (?, strftime('%s','now')) ON CONFLICT(key) DO UPDATE SET updated_at = excluded.updated_at`, ['paav6_learners']);
}
