...
export async function ensureSchema() {
  ...
  _schemaPromise = (async () => {
    try {
      ...
      // Run creation statements individually to isolate errors
      for (const sql of stmts) {
        try {
          await db.execute(sql);
        } catch (err) {
          console.warn(`[DB] Warning: Creation statement failed: ${sql.substring(0, 50)}...`, err.message);
        }
      }
      ...
      if (hasTable('learners')) {
        const lSchema = tables.rows.find(r => r.name === 'learners')?.sql || '';
      ...
      if (hasTable('staff')) {
        const sSchema = tables.rows.find(r => r.name === 'staff')?.sql || '';
        if (!sSchema.includes('avatar')) {
          try { await db.execute('ALTER TABLE staff ADD COLUMN avatar TEXT;'); } catch(e){}
        }
      }
      if (hasTable('paylog')) {
        const pSchema = tables.rows.find(r => r.name === 'paylog')?.sql || '';
      ...
      if (hasTable('subscriptions')) {
        const subSchema = tables.rows.find(r => r.name === 'subscriptions')?.sql || '';
        if (!subSchema.includes('amount')) {
          try { await db.execute('ALTER TABLE subscriptions ADD COLUMN amount REAL DEFAULT 0;'); } catch(e){}
        }
        if (!subSchema.includes('cycle')) {
