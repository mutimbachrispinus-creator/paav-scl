...
export async function kvUpdateStaffProfile(id, name, phone, avatar, password = null) {
  await ensureSchema();
  if (password) {
    await execute('UPDATE staff SET name = ?, phone = ?, avatar = ?, password = ? WHERE id = ?', [name, phone, avatar, password, id]);
  } else {
    await execute('UPDATE staff SET name = ?, phone = ?, avatar = ? WHERE id = ?', [name, phone, avatar, id]);
  }
  // Important: Touch the kv timestamp so the client-side cache knows to refresh the staff list
  await execute(`INSERT INTO kv (key, updated_at) VALUES (?, ?) ON CONFLICT(key) DO UPDATE SET updated_at = excluded.updated_at`, ['paav6_staff', Date.now()]);
}
