...
export async function kvBulkAddLearners(learners, tenantId = 'platform-master') {
  ...
  function isFuzzyMatch(n1, n2) {
    ...
    const w1 = s1.split(/\s+/);
    const w2 = s2.split(/\s+/);
    // If they share the first two words and are in the same grade, they are likely the same person
  ...
  for (const l of learners) {
    ...
    // Check against all existing for fuzzy match in same grade
    existing.forEach(old => {
      if (old.grade === lGrade && isFuzzyMatch(old.name, lName)) {
        if (old.adm !== l.adm) {
          admsToDelete.add(old.adm);
        }
      }
    });
  ...
  // 2. Remap associated data (marks, payments, etc.) before deleting the old duplicates
  if (admsToDelete.size > 0) {
    ...
    for (const l of learners) {
      ...
      // B. DEEP RECOVERY: Also check if the incoming ADM itself has orphaned data
      // (This handles cases where the learner record was deleted but marks remain)
      remapStmts.push({ sql: `UPDATE OR IGNORE marks SET adm = ? WHERE adm = ? AND tenant_id = ? AND adm NOT IN (SELECT adm FROM learners WHERE tenant_id = ?)`, args: [l.adm, l.adm, tenantId, tenantId] });
    }
    ...
    // Now safe to delete the old learner profiles
