  ...
  for (const [t, pk] of Object.entries(allTables)) {
    if (hasTable(t)) {
      ...
      if (schemaRow && !schemaRow.sql.includes(`PRIMARY KEY(${pk})`)) {
        ...
        // Generate new SQL with correct PK
        // This is a simplified migration: rename, create new, copy, drop.
        // We use the existing table definition but replace the PK.
        try {
          await db.batch([
            `ALTER TABLE ${t} RENAME TO ${t}_old`,
            oldSql.replace(/PRIMARY KEY\s*\([^)]+\)/gi, `PRIMARY KEY(${pk})`),
