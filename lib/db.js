...
export async function kvBulkAddLearners(learners, tenantId = 'platform-master') {
  ...
  // 1. Fetch existing learners to detect duplicates by Name + Grade
  const existing = await query('SELECT adm, name, grade FROM learners WHERE tenant_id = ?', [tenantId]);
  const nameGradeMap = new Map();
  existing.forEach(l => {
    const key = `${l.name.toUpperCase()}|${l.grade}`;
    if (!nameGradeMap.has(key)) nameGradeMap.set(key, []);
    nameGradeMap.get(key).push(l.adm);
  });
  ...
  for (const l of learners) {
    ...
    const lKey = `${lName}|${lGrade}`;
    
    // Check if this Name+Grade already exists with a DIFFERENT ADM
    const existingAdms = nameGradeMap.get(lKey) || [];
    existingAdms.forEach(oldAdm => {
      if (oldAdm !== l.adm) {
        admsToDelete.add(oldAdm);
      }
    });
  ...
  // 2. Delete the old duplicates before inserting/updating new ones
