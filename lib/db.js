...
export async function ensureSchema() {
  ...
  try {
    ...
    // 🚀 Step 1: Lightweight check to see if the database is already initialized
    const hasKv = await query("SELECT name FROM sqlite_master WHERE type='table' AND name='kv'").catch(() => []);
    if (hasKv.length > 0) {
      const verRow = await query('SELECT value FROM kv WHERE key = "schema_version" AND tenant_id = "platform-master"').catch(() => []);
      if (verRow.length > 0 && Number(verRow[0].value) >= 1) {
        console.log('[DB] Schema already at version 1. Skipping initialization.');
        _schemaChecked = true;
        return;
      }
    }

    console.log('[DB] Verifying/Initializing database schema...');
    ...
    // Only run migrations and repairs if we still haven't reached the target version
    ...
    await execute('INSERT OR REPLACE INTO kv (key, tenant_id, value, updated_at) VALUES (?, ?, ?, strftime("%s","now"))', ['schema_version', 'platform-master', '1']).catch(() => {});
