...
export async function ensureSchema() {
  ...
  for (const [t, pk] of Object.entries(allTables)) {
    if (hasTable(t)) {
      ...
      try { await db.execute(`ALTER TABLE ${t} ADD COLUMN tenant_id TEXT NOT NULL DEFAULT 'platform-master';`); } catch(e){}
      
      // 2. Check if primary key is compound. SQLite cannot ALTER primary keys.
      // We must recreate the table if the schema doesn't have the compound PK.
      const schemaRow = tables.rows.find(r => r.name === t);
      if (schemaRow && !schemaRow.sql.includes(`PRIMARY KEY(${pk})`)) {
        console.log(`[DB] Migrating table ${t} to compound primary key (${pk})...`);
        const oldSql = schemaRow.sql;
        try {
          let newSql = oldSql
            .replace(/PRIMARY KEY\s*\([^)]+\)/gi, '') // Remove compound PK
            .replace(/\s+PRIMARY KEY/gi, '')         // Remove inline PK
            .trim();
          
          // Remove trailing commas before adding the new PK
          newSql = newSql.replace(/,\s*\)$/, ')').replace(/\)$/, `, PRIMARY KEY(${pk}))`);
          // Clean up any double commas caused by the replacements
          ...
          console.log(`[DB] Table ${t} migrated successfully.`);
        } catch (migErr) {
          console.error(`[DB] Migration failed for ${t}:`, migErr);
        }
      }
    }
  }
...
export async function logAction(user, action, details) {
  const tenantId = user?.tenantId || 'platform-master';
