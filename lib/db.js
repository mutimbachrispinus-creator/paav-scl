...
export function getClient() {
  ...
  const url = (typeof process !== 'undefined' ? process.env.TURSO_URL : null) || 'https://placeholder.turso.io';
  const token = (typeof process !== 'undefined' ? process.env.TURSO_TOKEN : null) || '';

  // Optimized for Edge/Cloudflare
  const safeUrl = url.startsWith('libsql://') ? url.replace('libsql://', 'https://') : url;
  
  try {
    _client = createClient({ url: safeUrl, authToken: token });
    return _client;
  } catch (e) {
...
export async function query(sql, args = []) {
  return (await getClient().execute({ sql, args })).rows;
}
...
export async function ensureSchema() {
  // FAST PATH: Disable synchronous auto-migrations on Cloudflare Edge to prevent 500 timeouts.
  // The database schema is already initialized on Turso.
  // Running a blocking SELECT query on every cold start causes severe CPU spikes.
  _schemaChecked = true;
  return;
}

// Retained for manual/explicit migrations if needed in the future
export async function forceInitSchema() {
  try {
    const verRow = await query('SELECT value FROM kv WHERE key = "schema_version" AND tenant_id = "platform-master"').catch(() => []);
    if (verRow.length > 0 && Number(verRow[0].value) >= 2) return;
    
    console.log('[DB] Initializing schema (Manual Override)...');
    const tableStmts = [
    ...
    console.warn('[DB] Manual schema init warning:', e.message);
  }
}
