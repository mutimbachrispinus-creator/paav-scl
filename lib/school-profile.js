...
export function readSchoolProfile() {
  ...
  try {
    ...
    // Auto-Sanitize only the exact legacy placeholder if found in cache
    if (p.name === 'PAAV-Gitombo Community School') {
      p.name = 'EduVantage School Management System';
    }
    if (p.logo === '/ev-brand-v3.png' || !p.logo) {
      p.logo = '/ev-brand-v3.png';
    }
...
export function writeSchoolProfileCache(profile) {
  ...
  try {
    ...
    localStorage.setItem(CACHE_KEY, JSON.stringify({ v: profile, t: stamp, s: stamp }));
  } catch {}
}
