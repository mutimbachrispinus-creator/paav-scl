...
import { ALL_GRADES, gInfo, DEFAULT_SUBJECTS, maxPts } from '@/lib/cbe';
...
function ReportCardTemplate({ learners, subjects, marks, grade, term }) {
  return (
    <div className="rc-batch">
      {learners.map(l => (
        <div key={l.adm} style={{ pageBreakAfter: 'always', padding: 20, border: '2px solid #eee', marginBottom: 40 }}>
          <PrintHeader title="STUDENT PROGRESS REPORT" grade={grade} />
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 20, background: '#F8FAFF', padding: 15, borderRadius: 8 }}>
            <div><strong>Name:</strong> {l.name}</div>
            <div><strong>Admission No:</strong> {l.adm}</div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {subjects.map(s => {
                const score = marks[`${term}:${grade}|${s}|et1`]?.[l.adm];
                const info = score !== undefined ? gInfo(score, grade) : null;
                return (
                  <tr key={s} style={{ borderBottom: '1px solid #eee' }}>
                    <td style={{ padding: 10 }}>{s}</td>
                    <td style={{ padding: 10, textAlign: 'center' }}>{score ?? '—'}</td>
                    <td style={{ padding: 10, textAlign: 'center' }}>{info?.lv ?? '—'}</td>
                    <td style={{ padding: 10, textAlign: 'center' }}>{info?.pts ?? '—'}</td>
                  </tr>
                );
              })}
            </tbody>
