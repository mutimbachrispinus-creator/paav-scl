...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/db', {
      ...
      body: JSON.stringify({ requests: [{ type: 'get', key: 'paav_hero_img' }] })
    }).then(r => r.json()).then(data => {
      if (data.results?.[0]?.value) setHeroImg(data.results[0].value);
    }).catch(() => {});
    ...
    <div id="auth" style={{ minHeight: '100vh', display: 'flex', alignItems: 'stretch',
      ...
      <div className="auth-left" style={{ flex: 1, display: 'flex', flexDirection: 'column',
        ...
        <div className="auth-bg" style={{ 
          background: heroImg ? 'rgba(5, 15, 28, 0.65)' : 'transparent',
